...
En la siguiente tabla se muestran los commits realizados en la organización AcuaNode durante el Sprint 3, distribuidos entre el Edge API, el firmware embebido, el backend y el frontend.

| Repositorio | Branch | Commit Id | Commit Message | Commit Message Body | Commited on (Date) |
| :--- | :--- | :--- | :--- | :--- | :--- |
| AcuaNode/yaku-edge | main | `90e4bd0` | feat: add /queue endpoint to mosquitto broker | feat: add /queue endpoint to mosquitto broker | 2026-07-05 |
| AcuaNode/yaku-edge | main | `a33d021` | feat: iot connection via mobile hotspot | feat: iot connection via mobile hotspot | 2026-07-04 |
| AcuaNode/yaku-edge | main | `05d89e9` | feat: add flask server to convert HTTP -> MQTT -> HTTP | feat: add flask server to convert HTTP -> MQTT -> HTTP | 2026-07-03 |
| AcuaNode/yaku-edge | main | `600ad37` | feat: inital edge setup mosquitto + python + redis | feat: inital edge setup mosquitto + python + redis | 2026-06-27 |
| AcuaNode/yaku-embedded | master | `2e3b7f9` | feat: Add yaku embedded app using modestIoT arquitechture | feat: Add yaku embedded app using modestIoT arquitechture | 2026-07-05 |
| AcuaNode/yaku-backend | develop | `f070bef` | fix: allow Azure Storage static site origins in CORS config | fix: allow Azure Storage static site origins in CORS config | 2026-07-05 |
| AcuaNode/yaku-backend | develop | `11f651d` | fix: allow null ica column to fix schema migration on existing rows | fix: allow null ica column to fix schema migration on existing rows | 2026-07-05 |
| AcuaNode/yaku-backend | develop | `c050500` | fix: configure springdoc and forward headers for Swagger on Azure | fix: configure springdoc and forward headers for Swagger on Azure | 2026-07-05 |
| AcuaNode/yaku-backend | develop | `5698ca0` | fix: disable mqtt in test context to prevent bean instantiation error | fix: disable mqtt in test context to prevent bean instantiation error | 2026-07-05 |
| AcuaNode/yaku-backend | develop | `f032b2f` | fix: skip MqttPublisherConfig in tests via ConditionalOnProperty | fix: skip MqttPublisherConfig in tests via ConditionalOnProperty | 2026-07-05 |
| AcuaNode/yaku-backend | develop | `7834094` | feat: modify equipment and telemetry to improve iot connection | feat: modify equipment and telemetry to improve iot connection | 2026-07-04 |
| AcuaNode/yaku-frontend-flutter | main | `6e453fb` | feat: add iot connection with metrics (ported from feat/microservice) | feat: add iot connection with metrics (ported from feat/microservice) | 2026-07-05 |
| AcuaNode/yaku-frontend-flutter | main | `379ef71` | fix: delete ph variable | fix: delete ph variable | 2026-07-04 |
| AcuaNode/yaku-frontend-flutter | main | `ae3374f` | fix(config): allow overriding API base URL via --dart-define for prod web builds | fix(config): allow overriding API base URL via --dart-define for prod web builds | 2026-06-18 |
| AcuaNode/yaku-frontend-flutter | main | `7c1fbc8` | fix(admin): use real species threshold ranges in pond telemetry cards instead of hardcoded values | fix(admin): use real species threshold ranges in pond telemetry cards instead of hardcoded values | 2026-06-17 |
| AcuaNode/yaku-frontend-flutter | main | `85d0256` | fix(telemetry): replace oxygen sensor with turbidity and use real species thresholds for alerts | fix(telemetry): replace oxygen sensor with turbidity and use real species thresholds for alerts | 2026-06-17 |
| AcuaNode/yaku-frontend-flutter | main | `ae6730b` | feat(admin): add parametros page with species thresholds GET/POST and validation | feat(admin): add parametros page with species thresholds GET/POST and validation | 2026-06-14 |
| AcuaNode/yaku-frontend-flutter | main | `7a83c99` | feat(operator): add equipment tab, fix species dropdown and remove sin finca label | feat(operator): add equipment tab, fix species dropdown and remove sin finca label | 2026-06-14 |

<div style="page-break-after: always;"></div>

#### 6.2.3.5. Testing Suite Evidence for Sprint Review

**Estrategia de Pruebas - Sprint 3 (YakuControl)**

Para el Sprint 3, la estrategia de pruebas se amplió para cubrir el nuevo componente Edge API (Python/Flask) y el firmware embebido, además de mantener la suite de pruebas del backend consolidada en los sprints anteriores.
...
Se implementaron pruebas de integración para validar el flujo HTTP → MQTT → HTTP del Edge API:

```gherkin
Feature: Ingesta de datos via Edge API (TS04)
  Como dispositivo IoT, deseo enviar mis lecturas a traves del Edge API

  Scenario: Publicacion exitosa de lectura en el broker Mosquitto
    Given que el dispositivo envia una lectura valida via HTTP
    When el Flask server la recibe en el endpoint /queue
    Then la lectura se publica correctamente en el topico MQTT correspondiente
    And se almacena temporalmente en Redis para su posterior envio al backend

  Scenario: Conexion del dispositivo via hotspot movil
    Given que el dispositivo IoT se conecta a una red movil (hotspot)
    When intenta publicar una lectura
    Then el Edge API la recibe sin perdida de datos
    And mantiene la sesion activa pese a la variabilidad de la señal
```
...
- Pantallas mobile con datos reales.

<img src="./assets/sprint3/front1.png" width="45%">
<img src="./assets/sprint3/front2.png" width="45%">
<img src="./assets/sprint3/front3.png" width="45%">
<img src="./assets/sprint3/front4.png" width="45%">

- Consola de Arduino IDE

<img src="./assets/sprint3/embeded1.png" width="75%"><br>
<img src="./assets/sprint3/embeded2.png" width="75%">

- Datos enviados al edge.

<img src="./assets/sprint3/edge.png" width="75%">

-Brocker Mosquito. 

<img src="./assets/sprint3/brockermosquito0.png" width="75%">
<img src="./assets/sprint3/brockermosquito1.png" width="75%">

#### 6.2.3.7. Services Documentation Evidence for Sprint Review

En esta sección se documentan los endpoints del **Edge API** (`yaku-edge`), microservicio en Python (Flask + Mosquitto + Redis) desplegado en Railway, que actúa como puente entre el firmware embebido (`yaku-embedded`) y el backend principal alojado en Azure. Este servicio recibe la telemetría vía HTTP, la publica internamente en el broker MQTT, la persiste temporalmente en Redis para tolerar caídas de conectividad, y sincroniza los datos hacia el backend cuando la conexión a internet está disponible.

**API Endpoints — Edge API (Sprint 3)**

| Componente | Method | Endpoint | Puerto | Description |
| :--- | :--- | :--- | :--- | :--- |
| **Edge API (Flask)** | POST | `/ingest` | 5000 | Recibe la lectura de telemetría enviada por el dispositivo IoT (Arduino UNO + ESP8266) vía HTTP. Publica el dato internamente en el tópico MQTT `yaku/telemetria/pond/{id}` y responde al dispositivo con la configuración de umbrales vigente (`CFG:maxTemp,maxTurb`) y, de existir, un comando remoto pendiente para los actuadores (`PUMP1_ON` / `PUMP2_ON`). |
| **Edge API (Flask)** | GET | `/queue` | 5000 | Expone el contenido actual de la cola de sincronización en Redis (`telemetry_queue`), permitiendo inspeccionar los datos pendientes de envío hacia el backend en la nube. Retorna la cantidad de elementos y el detalle de cada lectura en cola. |
| **Broker MQTT (Mosquitto)** | SUB | `yaku/telemetria/pond/#` | 1883 | Tópico principal donde se publica la telemetría cruda de cada estanque. El comodín `#` permite escuchar los datos de todos los estanques registrados. |
| **Broker MQTT (Mosquitto)** | SUB | `yaku/config/thresholds/{species}` | 1883 | Tópico mediante el cual el backend publica los umbrales óptimos (temperatura y turbidez máximas) configurados por especie; el Edge API los persiste en Redis (`config:species:{species}`) para responder al dispositivo sin depender de la nube en cada ciclo. |
| **Broker MQTT (Mosquitto)** | SUB | `yaku/config/devices/{deviceId}` | 1883 | Tópico de enrutamiento que asocia un `deviceId` físico con la especie/estanque configurado, persistido en Redis (`route:device:{deviceId}`). |
| **Broker MQTT (Mosquitto)** | SUB | `yaku/command/devices/{deviceId}` | 1883 | Tópico de comandos remotos (ej. activar bomba de emergencia) enviados hacia un dispositivo específico. El comando se almacena en Redis con expiración de 30 segundos (`cmd:device:{deviceId}`) y se entrega en la siguiente respuesta HTTP al dispositivo. |
| **Redis** | — | `telemetry_queue` | 6379 | Cola FIFO en memoria que almacena temporalmente las lecturas antes de ser sincronizadas al backend, garantizando resiliencia ante cortes de conectividad en zonas rurales. |
| **Backend (destino de sincronización)** | POST | `/api/v1/telemetry/manual-ingest` | — | Endpoint del backend principal (Spring Boot / Azure) hacia el cual el proceso `sync_to_cloud()` del Edge API reenvía cada lectura ya traducida al formato esperado por el dominio (`deviceId`, `temperature`, `turbidity`, `ica`). |

### Payload de telemetría (Firmware → Edge API)

El dispositivo embebido (`AcuaNodeDevice::sendToEdge`) envía cada lectura mediante una petición `POST /ingest` con el siguiente cuerpo JSON:
...
- Edge API: [https://github.com/AcuaNode/yaku-edge](https://github.com/AcuaNode/yaku-edge)
- Firmware embebido: [https://github.com/AcuaNode/yaku-embedded](https://github.com/AcuaNode/yaku-embedded)
...
El siguiente gráfico muestra el historial de commits registrado en el repositorio del Edge API durante el Sprint 3, correspondiente a la implementación del puente HTTP → MQTT → Redis y su configuración inicial con Docker Compose.

<img src="./assets/sprint3/commids1.png" width="75%">

**Repositorio Backend**

El siguiente gráfico muestra la actividad de commits en el repositorio del Backend durante el Sprint 3, reflejando los ajustes realizados para la integración con el Edge API, así como las correcciones de configuración necesarias para el correcto funcionamiento en el entorno de producción de Azure.

<img src="./assets/sprint3/commids2.png" width="75%">
...
# Anexos
-Video de Expo Tb1:
- [Ver video de exposición tb1](https://upcedupe-my.sharepoint.com/:v:/g/personal/u202311064_upc_edu_pe/IQDr_G4JDYGgRZKS7ojHPay1AVABkjLnyQHjPSYEtCnzIPo?e=8OlaAS&nav=eyJyZWZlcnJhbEluZm8iOnsicmVmZXJyYWxBcHAiOiJTdHJlYW1XZWJBcHAiLCJyZWZlcnJhbFZpZXciOiJTaGFyZURpYWxvZy1MaW5rIiwicmVmZXJyYWxBcHBQbGF0Zm9ybSI6IldlYiIsInJlZmVycmFsTW9kZSI6InZpZXcifX0%3D)

-Frontend Desplegado:
- [Ver frontend](https://yakufrontend.z13.web.core.windows.net/)
